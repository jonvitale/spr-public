  ...
  <div>
    <Columns>
      ...
      <Column side="right" width="2/3">
        ...
        <div v-for="metricId in applicableMetricIds" :key="metricId">
...
export default {
  ...
  methods: {
    ...
    lookupTextByMultipleFieldValues(
      sourceFieldValueMap,
      targetField,
      returnMultiple
    ) {
      if (this.metricValues) {
        const values = this.$qlik.lookupValueByMultipleFieldValues(
          this.metricValues,
          sourceFieldValueMap,
          targetField,
          returnMultiple
    ...
    async update() {
      if (this.sessionObject) {
        ...
        this.pointsEarnedProjected = new Array(
          this.metricValues.metricId.length
        )
        this.pointsEarnedProjectedCalculated = new Array(
          this.metricValues.metricId.length
        )
        this.possiblePointsProjected = new Array(
          this.metricValues.metricId.length
        )
    ...
    resetProjectedMetric(metricId) {
      ...
      const metricIndex = this.$qlik.lookupIndexByFieldValue(
        this.metricValues,
        'metricId',
        metricId,
        false
    ...
    updateProjectedValuesByMetric(metricId, scoreProjected, doRollup) {
      const metricIndex = this.$qlik.lookupIndexByFieldValue(
        this.metricValues,
        'metricId',
        metricId,
    ...
    updateProjectedValuesAtIndex(
      ...
      const possiblePoints = this.metricValues.possiblePoints[metricIndex]
        .number
      const floor =
        this.metricValues.floorPoints[metricIndex].number / (isAgi ? 1 : 100)
      const target =
        this.metricValues.targetPoints[metricIndex].number / (isAgi ? 1 : 100)
    ...
    updateProjectedRollUpValues(domain, useDefaultValues) {
      ...
      const domainIndex = this.$qlik.lookupIndexByFieldValue(
        this.metricValues,
        'metricId',
        domainId,
        false
